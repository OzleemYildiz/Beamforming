...
%No contiguity and I need to come from different ends to make sure the
%contiguity

%But how the fuck we generalize this for more  Nrfs??


function [beam_loc, n_steps] = parallel_gt_ack_5g_2(total_codebook,n,m, valid_loc, beam_loc, location, n_steps, gain_gaussian, angle_ue, threshold)
%     if n == m
...
%            end
           if m ==0
            ...
            n = n-1;
            %ACK is 1
            pathexists_ex_1 = beamform_hierarchical(total_codebook,n, valid_loc(ex), gain_gaussian, angle_ue, threshold);
            ...
            if ex+1 <= length(valid_loc)
                ...
                pathexists_ex_2 = beamform_hierarchical(total_codebook,n, valid_loc(ex+1), gain_gaussian, angle_ue, threshold);
       ...
       if size_check >=n
        ...
        if size_check >length(valid_loc)
            fprintf('error\n');
        end
        ...
        if size_check == 1 
            if pathexists_1 && pathexists_2 && hold ~= 0
                beam_loc = [beam_loc, [valid_loc(1), valid_loc(end-hold+1:end)]];
            ...
            [beam_loc, n_steps] = parallel_gt_ack_5g_2(total_codebook,n,m, sort(valid_loc), beam_loc, location, n_steps, gain_gaussian, angle_ue, threshold); 
        ...
        elseif pathexists_1 && pathexists_2 && size_check~=1 % Both side has 1
            ...
            valid_loc = [valid_loc_1, valid_loc(size_check+1:end-hold),valid_loc_2];
        ...
        elseif pathexists_1==0 && pathexists_2 && size_check~=1 % Second one has ACK
            ...
            hold_2 =  length(valid_loc(1:end-hold));
            ...
            if hold_2 ~= 0
                ...
                pathexists_in = beamform_hierarchical(total_codebook,n, valid_loc(1:hold_2), gain_gaussian, angle_ue, threshold);
                ...
                if pathexists_in==0 %NACK
                 ...
                 if isempty(valid_loc(hold_2+1:end-hold))==0 %Not empty so let me check
                    size_more = 2^floor(log2(length(valid_loc(hold_2+1:end-hold))));
                    %check_more = sum(location(valid_loc(hold+1+hold_2:end)) == 0)== size_more; %NACK
                    pathexists_more = beamform_hierarchical(total_codebook,n, flip(valid_loc(end-hold-size_more+1:end-hold)), gain_gaussian, angle_ue, threshold);
                    ...
                    elseif pathexists_more && size_more ==1 %ACK and size 1
                        beam_loc = [beam_loc,  valid_loc(hold_2+1:end-hold)];
                        ...
                        valid_loc = [valid_loc(hold_2+1:end-hold), valid_loc_2];
                        n = n-hold_2;
                    end 
                    ...
                    [beam_loc, test_n_in, n,m, valid_loc_in] = binary_split_5g(total_codebook,n, m, location, valid_loc(1:hold_2),hold_2, beam_loc, 0, gain_gaussian, angle_ue, threshold);
                    valid_loc = [valid_loc_2, valid_loc_in,  valid_loc(+1+hold_2:end-hold)];
        ...
        elseif pathexists_1 && pathexists_2==0  && size_check~=1 % First one has ACK
            ...
            if hold_2 ~= 0
                ...
                pathexists_in = beamform_hierarchical(total_codebook,n, flip(valid_loc(end-hold_2+1:end)), gain_gaussian, angle_ue, threshold);

                
                n_test_in = 1;
                if pathexists_in==0 %NACK
                    
                    if isempty(valid_loc(size_check+1:end-hold_2))==0 %Not empty so let me check
                        size_more = 2^floor(log2(length(valid_loc(size_check+1:end-hold_2))));
                        %check_more = sum(location(valid_loc(size_check+1+hold_2:end)) == 0)== size_more; %NACK
                        pathexists_more = beamform_hierarchical(total_codebook,n, flip(valid_loc(end-hold_2-size_more+1:end-hold_2)), gain_gaussian, angle_ue, threshold);
                        ...
                        if pathexists_more==0 % The rest is also NACK
                            ...
                            n = n-hold_2 -size_more; 
                        elseif pathexists_more && size_more ==1 %ACK and size 1
                            beam_loc = [beam_loc,  valid_loc(size_check+1:end-hold_2)];
                            valid_loc = valid_loc_2; %The size was 1 and the hold gave NACK so binary split;
                            n = n-hold_2-1; %1 beam we found
                            m = m-1; 
                        else %ACK but not size 1
                            valid_loc = [valid_loc_2, valid_loc(size_check+1:end-hold_2)];
                            n = n-hold_2;
                        end 
                    else %The rest is empty
                        valid_loc = valid_loc_2;% Just use Binary splitting result earlier
                    ...
                    [beam_loc, test_n_in, n,m, valid_loc_in] = binary_split_5g(total_codebook,n, m, location, flip(valid_loc(end-hold_2+1:end)),hold_2, beam_loc, 0, gain_gaussian, angle_ue, threshold);
                    valid_loc = [valid_loc_2, valid_loc_in, valid_loc(size_check+1:end-hold_2)];
                    n_test_in = n_test_in + test_n_in;
                end
            else
                valid_loc = valid_loc_2;
