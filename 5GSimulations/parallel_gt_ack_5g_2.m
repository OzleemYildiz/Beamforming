...
%            end
       ...
       if size_check >=n
        ...
        if size_check == 1 
            ...
            [beam_loc, n_steps] = parallel_gt_ack_5g_2(total_codebook,n,m, valid_loc, beam_loc, location, n_steps, gain_gaussian, angle_ue, threshold); 
            ...
            if hold_2 ~= 0
                ...
                if pathexists_in==0 %NACK
                 ...
                 if isempty(valid_loc(hold_2+1:end-hold))==0 %Not empty so let me check
                    size_more = length(valid_loc(hold_2+1:end-hold));
                    %check_more = sum(location(valid_loc(hold+1+hold_2:end)) == 0)== size_more; %NACK
                    pathexists_more = beamform_hierarchical(total_codebook,n, valid_loc(hold_2+1:end-hold), gain_gaussian, angle_ue, threshold);
                    ...
                    elseif pathexists_more && size_more ==1 %ACK and size 1
                        ...
                        valid_loc = [valid_loc_2, valid_loc(hold_2+1:end-hold)];
                        n = n-hold_2;
                    end 
        ...
        elseif pathexists_1 && pathexists_2==0  && size_check~=1 % First one has ACK
            ...
            if hold_2 ~= 0
                ...
                pathexists_in = beamform_hierarchical(total_codebook,n, valid_loc(end-hold_2+1:end), gain_gaussian, angle_ue, threshold);
                ...
                if pathexists_in==0 %NACK
                    ...
                    if isempty(valid_loc(size_check+1:end-hold_2))==0 %Not empty so let me check
                        size_more = length(valid_loc(size_check+1:end-hold_2));
                        %check_more = sum(location(valid_loc(size_check+1+hold_2:end)) == 0)== size_more; %NACK
                        pathexists_more = beamform_hierarchical(total_codebook,n, valid_loc(size_check+1:end-hold_2), gain_gaussian, angle_ue, threshold);
                    ...
                    [beam_loc, test_n_in, n,m, valid_loc_in] = binary_split_5g(total_codebook,n, m, location, valid_loc(end-hold_2+1:end),hold_2, beam_loc, 0, gain_gaussian, angle_ue, threshold);
