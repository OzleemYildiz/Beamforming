...
%             number_of_test_freq_Hwang(j,i) = number_of_test_freq_Hwang(j,i) + n_steps;
              ...
              [beam_loc_2, n_steps_2] = Hwang_gt(floor(n(i)/2), m(j),1:ceil(n(i)/2),{}, location(ceil(n(i)/2)+1:end), 0);
              %Dont forget beam_loc_2 +16 is the actual result
              n_steps = max(n_steps_1, n_steps_2);
              number_of_test_freq_Hwang_2seperate(j,i) = number_of_test_Hwang(j,i) + n_steps;
